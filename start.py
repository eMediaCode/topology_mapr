...
def main(args):
    ...
    while time() < stop_time:
        exit_code = primary_node.execute('service mapr-warden status').exit_code
        if exit_code == 0:
            break
        sleep(1)
    
    if exit_code != 0:
        raise Exception('Service MapR warden is still not running. Exiting.')
    ...
    def success(time):
        logger.info('MapR Control System server is online in %s seconds.', time)
    def failure(timeout):
